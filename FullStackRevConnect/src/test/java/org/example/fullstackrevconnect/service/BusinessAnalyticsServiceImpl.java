...
class BusinessAnalyticsServiceImplTest {
    ...
    @Test
    void testGetBusinessAnalytics_success() {

        Long userId = 1L;

        when(connectionRepository.countByReceiverIdAndStatus(userId, Connection.Status.ACCEPTED))
                .thenReturn(5L);

        when(connectionRepository.countBySenderIdAndStatusOrReceiverIdAndStatus(
                userId, Connection.Status.ACCEPTED,
                userId, Connection.Status.ACCEPTED))
                .thenReturn(10L);

        when(profileViewRepository.countByViewedUserId(userId))
                .thenReturn(20L);

        when(postRepository.getTotalImpressions(userId))
                .thenReturn(100L);

        Post post = mock(Post.class);
        when(post.getLikedUsers()).thenReturn(Collections.emptyList());

        when(postRepository.findByAuthorId(userId))
                .thenReturn(List.of(post));

        when(commentRepository.countCommentsForBusiness(userId))
                .thenReturn(7L);

        BusinessAnalyticsDTO dto = service.getBusinessAnalytics(userId);

        assertEquals(5L, dto.getTotalFollowers());
        assertEquals(10L, dto.getTotalConnections());
        assertEquals(20L, dto.getProfileViews());
        assertEquals(100L, dto.getPostImpressions());
        assertEquals(7L, dto.getPostEngagement());
        assertEquals(6, dto.getMonthlyGrowth().size());
    }
